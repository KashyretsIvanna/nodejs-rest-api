...
const removeContact = async (contactId) => {
  ...
  await fs.writeFile(__dirname + "/contacts.json", JSON.stringify(newArray), (err) => {
    if (err) {
      console.log(err);
    }
...
const addContact = async (body) => {
  ...
  fs.writeFile(__dirname + "/contacts.json", JSON.stringify([...contacts, body]), (err) => {
    console.log(err);
  });
...
const updateContact = async (contactId, body) => {
  ...
  await fs.writeFile(__dirname + "/contacts.json", JSON.stringify(array), (err) => {
    if (err) {
      console.log(err);
    }
  })
